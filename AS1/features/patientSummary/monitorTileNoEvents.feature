@regression @automated @core  @patient_summary @34

Feature: PM - Monitor Tile Active No Events

  Description
  Monitor - "No monitor data" Occurs when Patient is not active, has no PM data in Past 24 hrs
  (not on a monitor). Server will send status of “na” which client will have to translate into “No Monitor Data”

  -No Monitor Data
  Patient without Monitor data/Events in last 24h
  Monitor Tile - No messaging appears
  Event Tile section - "No Monitor Data" is the message centered in the tile

  TestRail Id: C264294
  Jira Stories: AS1-267, AS1-380
  @critical @TMS:264294 @mock
  Scenario: PM - Monitor Tile Active No Events
    Given I am a super user with all permissions
    When I login to a testSite with a valid credential
    Then I should see the patient list screen
    And I reset grouping and sorting on patients list
    And reset unit filtering
    Then I should see the patient list screen
    When I click "Census" on the List
    Then I should see the patient list screen
    When I click on "[props.NEPACTIVE_FULL_NAME]" in patient list
    Then I should see the patient summary screen
    And I should see the Monitor Tile as "active" with No Events
