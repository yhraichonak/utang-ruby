@regression @manual @automated @TMS:582450 @c3po
Feature: C3PO - Custom Bed MPV - 4 View
  TestRail Id: C582450
  Jira Epic: AS1-2518
  ...
  So that I can launch the 4 Monitor Multi Patient View for Custom Bed list
  ...
  - The "listIndex" parameter within the Query will also vary depending on the site used. When you go to the patient list,
  each option in the list of lists has an index. That index will be on the end of URL in the address bar.
  - When PHI is set to TRUE, the PHI information is visible. When the PHI is set to FALSE, the PHI information is hidden.

  Scenario: C3PO link opens 4 Monitor MPV for Custom Bed list with PHI visible
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_CUSTOM_PATIENT_LIST_ID]&view=4&zoom=5&monitorZoom=2&phi=true
  ...
  Scenario: C3PO link opens 4 Monitor MPV for Custom Bed list with PHI hidden
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_CUSTOM_PATIENT_LIST_ID]&view=4&zoom=5&monitorZoom=2&phi=false
    And I navigate to the generated C3PO url
    Then I should see "4" condensed monitors on c3po MPV screen
    And I should see patient names, zoom control, demographics, location, alarms, and 1 waveform if monitor screen displayed
