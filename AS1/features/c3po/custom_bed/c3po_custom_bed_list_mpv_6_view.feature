@regression @manual @automated @TMS:582451 @c3po
Feature: C3PO - Custom Bed MPV - 6 View
  TestRail Id: C582451
  Jira Epic: AS1-2518
  ...
  So that I can launch the 6 Monitor Multi Patient View for the Custom Bed list
  ...
  Scenario: C3PO link opens 6 Monitor MPV for Custom Bed list with PHI visible
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_CUSTOM_PATIENT_LIST_ID]&view=6&zoom=5&monitorZoom=2&phi=true
  ...
  Scenario: C3PO link opens 6 Monitor MPV for Custom Bed list with PHI hidden
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_CUSTOM_PATIENT_LIST_ID]&view=6&zoom=5&monitorZoom=2&phi=false
    And I navigate to the generated C3PO url
    Then I should see "6" condensed monitors on c3po MPV screen
    And I should see patient names, zoom control, demographics, location, alarms, and 1 waveform if monitor screen displayed
