@regression @manual @automated @TMS:582446 @c3po
Feature: C3PO - My Patients MPV - 12 View
  TestRail Id: C582446
  Jira Epic: AS1-2518
  ...
  So that I can launch the 12 Monitor Multi Patient View for the My Patients list
  ...
  Scenario: C3PO link opens 12 Monitor MPV for My Patients with PHI visible
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_MY_PATIENT_LIST_ID]&view=12&zoom=5&monitorZoom=2&phi=true
  ...
  Scenario: C3PO link opens 12 Monitor MPV for My Patients with PHI hidden
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_MY_PATIENT_LIST_ID]&view=12&zoom=5&monitorZoom=2&phi=false
    And I navigate to the generated C3PO url
    Then I should see "12" condensed monitors on c3po MPV screen
    And I should see each condensed monitor contains patients demographics, location, alarms, and 1 waveform
    When I click side navigation menu "Patient list" in MPV screen
