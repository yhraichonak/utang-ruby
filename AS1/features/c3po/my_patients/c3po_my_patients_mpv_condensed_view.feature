@regression @manual @automated @critical @TMS:582441 @c3po
Feature: C3PO - My Patients MPV - Condensed View
  TestRail Id: C582441
  Jira Epic: AS1-2518
  ...
  So that I can launch the Condensed Multi Patient View for the My Patients list
  ...
  Scenario: C3PO link opens Condensed MPV for My Patients with PHI visible
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=3&listIndex=0&view=0&zoom=5&monitorZoom=5&phi=true
...
#    # And I should see the Multi Patient View screen with condensed monitors "2" in size
  ...
  Scenario: C3PO link opens Condensed MPV for My Patients with PHI hidden
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=3&listIndex=0&view=0&zoom=5&monitorZoom=2&phi=false
    And I navigate to the generated C3PO url
    Then I should see "40" condensed monitors on c3po MPV screen
    And I should see each condensed monitor contains patients demographics, location, alarms, and 1 waveform
    When I click side navigation menu "Patient list" in MPV screen
