@regression @automated @TMS:582417 @c3po
Feature: C3PO - PM Census MPV - Condensed View
  TestRail Id: C582417
  Jira Epic: AS1-2518
  ...
  So that I can launch the Condensed Multi Patient View for the PM Census list
  ...
  parameters:
  siteid - (required)
  listIndex - (required) This index is defined on the web front-end. It can be used to select either patient-lists
  or custom-bed-lists This parameter will eventually be replaced by a listId defined on the server.
  ...
  @critical
  Scenario: C3PO link opens Condensed MPV for PM Census with PHI visible
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_CENSUS_PATIENT_LIST_ID]&view=0&zoom=5&monitorZoom=2&phi=true
    And I navigate to the generated C3PO url
    Then I should see "40" condensed monitors on c3po MPV screen
  ...
  Scenario: C3PO link opens Condensed MPV for PM Census with PHI hidden
    Given I open C3PO URL generation interface
    And I generate pm-mpv ast C3PO url with query TEST=1234567890&ad=username&moduleId=[prop.COMMON_PM_MODULE_ID]&listIndex=[props.COMMON_CENSUS_PATIENT_LIST_ID]&view=0&zoom=5&monitorZoom=2&phi=false
    And I navigate to the generated C3PO url
    Then I should see "40" condensed monitors on c3po MPV screen
    And I should see each condensed monitor contains patients demographics, location, alarms, and 1 waveform
    When I click side navigation menu "Patient list" in MPV screen
