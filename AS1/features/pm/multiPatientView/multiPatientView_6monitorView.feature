...
Feature: Multi-Patient View - 6 View
6 View
Header:
...
Show 6 patient monitors, 2 rows of 3 monitors each (equally sized)
Reference precedence order of waveforms in Jira  AS1-2506: Snippets Waveform Analysis ToolOPEN OPEN (configuration story)

Monitors in MPV
The Monitor area in  6 View mode will display in a similar fashion to PM, aside from anything referenced as different in this story
...
AS1-3955
  MPV - Display 6+ views in rows of 3 by default (Web)
    All views above 4 should display by default in rows of 3.  
      6 view should be 2 rows of 3 monitors each
...
Background:
  ...
  And I have 6 or more patients added to My Patients List
  Then I should see the patient list screen
  ...
  @critical @TMS:580690
Scenario: Multi-Patient View - 6 View
  ...
  And I should see the 6 full monitors should split into 2 rows and 3 monitors on each row 
#  And waveforms are streaming in all 6 monitors (AS1-5720)
