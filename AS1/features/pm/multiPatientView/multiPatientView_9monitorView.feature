...
Feature: Multi-Patient View - 9 View
8 View
Header:
The header includes Group/Sort, Filter Units, Multi-Patient View(left aligned)
Zoom Control (Single lead only) Condensed PM Box Control (single lead only), Split Screen View Control, Quad View Control, 6 view control, 9 view control, search (right aligned)
...
-The user can ONLY view these monitors in live mode - no historical mode will be allowed (panning)

AS1-3955
  MPV - Display 6+ views in rows of 3 by default (Web)
    All views above 4 should display by default in rows of 3. 
      8 view should now be 9 view and display three rows of 3 monitors each
...
Background:
  ...
  And I have 9 or more patients added to My Patients List
  Then I should see the patient list screen
...
@critical @TMS:580691
Scenario: Multi-Patient View - 9 View
  ...
  Then I should see 9 full monitors on the screen of the first 6 patients in previous screen
  And I should see the 9 full monitors should split into 3 rows and 3 monitors on each row
#  And waveforms are streaming in all 9 monitors (AS1-5720)
