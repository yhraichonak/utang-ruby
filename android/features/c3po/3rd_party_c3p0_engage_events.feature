@automated @TMS:328914  @c3p0
Feature: 3rd_party_c3p0_engage_events
  As a C3P0 user
...
#  Exception: When in backgrounded mode, the screen may briefly show where the user left-off, and then transition to the new context.
  ...
  Scenario: Engage - C3P0 launch to PM Events screen (Welcome Screen, Backgrounded Application)
    When I move app to background
    When I open pm-evt Engage C3PO url with payload [props.C3POA_WITHOUT_TIME_PAYLOAD]
    Then I should see the Events screen of [props.C3PO_FULL_NAME] patient

  Scenario: Engage - C3P0 launch to PM Events screen (Welcome Screen, Force Closed Application)
    When I force close app
    And I open pm-evt Engage C3PO url with payload [props.C3POA_WITHOUT_TIME_PAYLOAD]
    Then I should see the Events screen of [props.C3PO_FULL_NAME] patient

  Scenario: Engage - C3P0 launch to PM Events screen (Sites List, Backgrounded Application)
  ...
  Scenario: Engage - C3P0 launch to PM Events screen (Sites List, Force Closed Application)
    When I sign into AS1
    Then I should see the Site List screen
    When I force close app
    And I open pm-evt Engage C3PO url with payload [props.C3POA_WITHOUT_TIME_PAYLOAD]
