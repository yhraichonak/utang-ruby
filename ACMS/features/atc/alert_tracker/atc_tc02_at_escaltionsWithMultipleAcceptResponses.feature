...
@clear_atc_flights @TMS:584205
Feature: alertTracker_tc02_escalationsWithMultipleAcceptResponses
  ...
  Scenario: flight with three destination levels, a non zero delay on accept timeout, and multiple accept responses per level
    ...
    And FT: User waits unit the generated alert is present in alerts table with attributes "{ "Alarm Label":"TC02_Alarm", "End Time":"", "Delivered":"true", "Accepted":"false"}"
    ...
    Then FT: User verifies that the alert details panel has 6 events matching ".*[props.ACMS_TIMESTAMP_REGEXP].*AcceptReceived.*Accept Received. From.autoaccept.*"
