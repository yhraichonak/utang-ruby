...
@clear_atc_flights @TMS:584212
Feature: alertTracker_tc09_nonZeroInitialSuspendWithAlarmEnd
  ...
  So that I can verify an alarm that ends while in the initial suspend state will go into completed state after the cancellation suspense timeout duration passes
  ...
  Scenario: flight with a non-zero initial suspend value and with an alarm duration that is less than the initial suspend value
    ...
    And FT: User waits unit the generated alert is present in alerts table with attributes "{ "Alarm Label":"TC09_Alarm", "End Time":"", "Delivered":"false", "Accepted":"false"}"
