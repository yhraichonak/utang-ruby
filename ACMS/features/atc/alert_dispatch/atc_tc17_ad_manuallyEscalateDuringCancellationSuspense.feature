...
Feature: airDispatch_tc17_manuallyEscalateDuringCancellationSuspense
  As an Air Traffic Controller user
  I want to generate a flight that gets manually escalated after the alarm for the flight has ended and the flight's CancellationSuspense timeout is activated
  So that I can verify that an alarm can manually change escalation levels successfully with an activated CancellationSuspense timeout
  ...
  Scenario: flight with a manual escalation of alarm and with a non-zero cancellation suspense value
    ...
    And AC: User manually escalates alarm through elipses menu
