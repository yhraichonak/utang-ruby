...
Feature: airDispatch_tc18_autoAcceptEscalationDuringCancellationSuspense
  As an Air Traffic Controller user
  I want to generate a flight that gets auto accepted by the recipient after the alarm for the flight has ended and the flight's CancellationSuspense timeout is activated
  So that I can verify that an accepted response from a recipient extends the escalation during an activated CancellationSuspense timeout
  ...
  Scenario: flight with an auto accept recipient and with a non-zero cancellation suspense value
    ...
    And AC: User sees the elapsed flight time on the flight entry stop at "1:10" within 3 second range
    ...
    And AC: User verifies that the alert details panel has event matching ".*[props.ACMS_TIMESTAMP_REGEXP].*DispatchSent. *Alert Sent. Address:autoaccept*"
#    And AC: User verifies that the alert details panel has event matching ".*[props.ACMS_TIMESTAMP_REGEXP].*NotificationSent. *Notification Sent. Destination:Destination2_SingleRec.*"
    ...
    And AC: User verifies that the alert details panel has event matching ".*[props.ACMS_TIMESTAMP_REGEXP].*DispatchReply. *ResponseBy:autoaccept. Choice:Accept*"
    And AC: User verifies that the alert details panel has event matching ".*[props.ACMS_TIMESTAMP_REGEXP].*ExtendEscalation.*"
