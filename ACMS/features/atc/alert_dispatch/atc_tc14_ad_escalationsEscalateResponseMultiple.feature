...
Feature: airDispatch_tc14_escalationsEscalateResponseMultiple
  ...
  Scenario: flight with multiple recipients set to autoaccept notification, only first escalation accept is represented
    ...
    And AC: User verifies the generated alert is present in Alerts panel with attributes
      """
      { "assigned-to":"destination:[props.DESTINATIONS_DESTINATION2_SINGLEREC]", "escalates-to": "destination:none"}
      """
    ...
    And AC: User verifies that the alert details panel has 1 events matching ".*[props.ACMS_TIMESTAMP_REGEXP].*Flight Escalated. To Level:2. *Reason: EscalationResponse.*"
