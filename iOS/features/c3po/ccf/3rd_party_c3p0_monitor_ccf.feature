...
C3PO links to a patient context should open directly to the specified context without visual indication of the navigational elements in-between (such as site list).
        ...
        Scenario: CCF - C3P0 launch to PM Monitor screen (Welcome Screen, Backgrounded Application)
             When I move app to background
              And I open pm-mon CCF C3PO url with payload [props.C3PO_PM_MONITOR_WITHOUT_TIME_PAYLOAD]
             Then I should see the Live Monitor screen of [props.C3PO_FULL_NAME] patient

        Scenario: CCF - C3P0 launch to PM Monitor screen (Welcome Screen, Force Closed Application)
             When I force close app
              And I open pm-mon CCF C3PO url with payload [props.C3PO_PM_MONITOR_WITHOUT_TIME_PAYLOAD]
             Then I should see the Live Monitor screen of [props.C3PO_FULL_NAME] patient

        Scenario: CCF - C3P0 launch to PM Monitor screen (Sites List, Backgrounded Application)
        ...
        Scenario: CCF - C3P0 launch to PM Monitor screen (Sites List, Force Closed Application)
             When I sign into AS1
             Then I should see the Site List screen
             When I force close app
              And I open pm-mon CCF C3PO url with payload [props.C3PO_PM_MONITOR_WITHOUT_TIME_PAYLOAD]
