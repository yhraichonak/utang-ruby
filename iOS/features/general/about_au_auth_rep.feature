@regression @hotfix @automated @TMS:580681
Feature: About Screen AU Authorized Representative
              As an AS1 user
              I want to view the About Screen
  So that I can verify the About Information

TestRail Id: C580681

        Background:
            Given I verify the version of the app installed
            Given I should see the test site button on Site List screen

        Scenario: Verify the AU Authorized Representative Information
             When I click the gear button on Site List screen
              And I click the About button
             Then I should see the About screen
             When I scroll to the bottom of the view
             Then I verify the AU Authorized Representative Info on the About screen
